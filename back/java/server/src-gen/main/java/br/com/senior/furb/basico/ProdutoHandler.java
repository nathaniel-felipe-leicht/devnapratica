...
import static br.com.senior.furb.basico.ProdutoEntity.SECURITY_RESOURCE;
...
import br.com.senior.furb.basico.Produto.PageRequest;
import br.com.senior.furb.basico.Produto.PagedResults;
import br.com.senior.furb.basico.BasicoConstants;
import br.com.senior.furb.basico.CreateProduto;
import br.com.senior.furb.basico.UpdateProduto;
import br.com.senior.furb.basico.DeleteProduto;
import br.com.senior.furb.basico.RetrieveProduto;
import br.com.senior.furb.basico.ListProduto;
...
import br.com.senior.furb.basico.ProdutoDTOConverter;
import br.com.senior.platform.translationhub.api.TranslationHubApi;

@HandlerImpl(serviceClass = BasicoConstants.class)
@Component("furb.basico.ProdutoHandler")
public class ProdutoHandler implements CreateProduto, CreateMergeProduto, UpdateProduto, DeleteProduto, 
		 UpdateMergeProduto, CreateBulkProduto, RetrieveProduto, ListProduto, 
		 ImportProduto, ExportProduto {
	static final Logger logger = LoggerFactory.getLogger(ProdutoHandler.class);

	@Inject
	ProdutoCrudService service;
	
	@Inject
	ProdutoDTOConverter dtoConverter;
	...
	public Produto createProduto(Produto toCreate) {
		try {
			ProdutoEntity entity = service.createProduto(dtoConverter.toEntity(toCreate, ProdutoEntity.class));
			return dtoConverter.toDTO(entity, Produto.class);
	...
	@SecureResource(name=SECURITY_RESOURCE, action=ENTITY_SECURITY_ACTION_CREATE)
	@Override
	public Produto createMergeProduto(Produto toCreate) {
		try {
			ProdutoEntity entity = service.createProduto(dtoConverter.toEntity(toCreate, ProdutoEntity.class));
			return dtoConverter.toDTO(entity, Produto.class);
	...
	public Produto updateProduto(Produto toUpdate) {
		try {
			ProdutoEntity modified = doUpdate(toUpdate);
			return dtoConverter.toDTO(modified, Produto.class);
	...
	@SecureResource(name=SECURITY_RESOURCE, action=ENTITY_SECURITY_ACTION_UPDATE)
	@Override
	public Produto updateMergeProduto(Produto toUpdate) {
		try {
			ProdutoEntity entity = service.retrieveProduto(new Produto.Id(toUpdate.id));
			dtoConverter.mergeToEntity(toUpdate, entity);
			entity = service.updateProduto(entity);
			return dtoConverter.toDTO(entity, Produto.class);
	...
	private ProdutoEntity doUpdate(Produto toUpdate) {
		ProdutoEntity entity = service.getRepository().findOne(java.util.UUID.fromString(toUpdate.id));
		
		if(entity == null)
			throw new ServiceException(ErrorCategory.OBJECT_NOT_FOUND, translationHubApi.getString("br.com.senior.furb.basico.Produto_notFound", "Produto not found with id " + toUpdate.id));
		
		dtoConverter.toEntity(toUpdate, entity);
		return doUpdate(entity);
	}
	
	private ProdutoEntity doUpdate(ProdutoEntity entity) {
		return service.updateProduto(entity);
	}
	...
	@Override
	public Produto retrieveProduto(Id id) {
		ProdutoEntity entity = service.retrieveProduto(id);
		return dtoConverter.toDTO(entity, Produto.class);
	}
	...
	@Transactional(readOnly = true)
	@Override
	public PagedResults listProduto(PageRequest pageRequest) {
		...
		Page<ProdutoEntity> pages = this.service.listProdutoPageable(skip, top, orderBy, filter);
		List<ProdutoEntity> entities = pages.getContent();
		List<Produto> dtos = null;
		
		dtos = entities != null ? entities.stream().map(e -> dtoConverter.toDTO(e, Produto.class, pageRequest.displayFields)).collect(Collectors.toList()) : null;
	...
	public CreateBulkProdutoOutput createBulkProduto(CreateBulkProdutoInput toCreate) {
		if (toCreate != null) {
			List<Produto> entitiesToCreate = toCreate.entities;
			if (entitiesToCreate != null && !entitiesToCreate.isEmpty()) {
				entitiesToCreate.forEach(bean -> {
					bean.validate();
				});
				
				List<ProdutoEntity> entities = entitiesToCreate.stream().map(bean -> dtoConverter.toEntity(bean, ProdutoEntity.class))
						.collect(Collectors.toList());
				this.service.createBulk(entities);
			}
		}
		return new CreateBulkProdutoOutput();
	}
	...
	public ImportProdutoOutput importProduto(ImportProdutoInput request) {
		...
		ImportProdutoOutput output = new ImportProdutoOutput(importer.getId());
		return output;
	}
	...
	public ExportProdutoOutput exportProduto(ExportProdutoInput request) {
		 ...
		 */
		ExportProdutoOutput output = new ExportProdutoOutput(
				null /* exporter.getId() */);
		return output;
	}
	...
	class StartImportProcess extends Thread {
		...
		StartImportProcess(Importer importer, ImportConfig config) {
			...
			this.importer.addListener(new ProdutoImporterListener());
			this.config = config;
		...
		public void run() {
			...
			try {
				...
				try (FileReader reader = new FileReader(blob)) {
					...
					importer.setBeanClass(Produto.class.getName());
					importer.setCrudService(ProdutoHandler.this.service, (config.batchSize > 0));
		...
		private File getHttpBlobFile(URI uri) {
			...
			try (CloseableHttpClient httpClient = HttpClients.createDefault();
				...
				File blob = File.createTempFile("produto", "blob");
