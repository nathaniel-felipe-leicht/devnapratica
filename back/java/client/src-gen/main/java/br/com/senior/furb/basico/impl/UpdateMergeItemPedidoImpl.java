 ...
 * 
 */
public class UpdateMergeItemPedidoImpl {
	...
	public UpdateMergeItemPedidoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ItemPedido input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.UPDATE_MERGE_ITEM_PEDIDO, //
	...
	*/
	public ItemPedido updateMergeItemPedido(ItemPedido input, long timeout) {
		...
		ItemPedido output = DtoJsonConverter.toDTO(resultMessage.getContent(), ItemPedido.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ItemPedido> updateMergeItemPedidoRequest(ItemPedido input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ItemPedido.class);
	}
