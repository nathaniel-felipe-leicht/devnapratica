 ...
 * 
 */
public class UpdateMergeProdutoImpl {
	...
	public UpdateMergeProdutoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Produto input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.UPDATE_MERGE_PRODUTO, //
	...
	*/
	public Produto updateMergeProduto(Produto input, long timeout) {
		...
		Produto output = DtoJsonConverter.toDTO(resultMessage.getContent(), Produto.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<Produto> updateMergeProdutoRequest(Produto input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Produto.class);
	}
