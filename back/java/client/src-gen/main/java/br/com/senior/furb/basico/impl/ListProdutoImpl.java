...
public class ListProdutoImpl {
	...
	public ListProdutoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Produto.PageRequest input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.LIST_PRODUTO, //
	...
	*/
	public Produto.PagedResults listProduto(Produto.PageRequest input, long timeout) {
		...
		Produto.PagedResults output = DtoJsonConverter.toDTO(resultMessage.getContent(), Produto.PagedResults.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<Produto.PagedResults> listProdutoRequest(Produto.PageRequest input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Produto.PagedResults.class);
	}
