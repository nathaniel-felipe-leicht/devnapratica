...
public class GetSaldoImpl {
	...
	public GetSaldoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(GetSaldoInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.GET_SALDO, //
	...
	*/
	public GetSaldoOutput getSaldo(GetSaldoInput input, long timeout) {
		...
		GetSaldoOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), GetSaldoOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<GetSaldoOutput> getSaldoRequest(GetSaldoInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, GetSaldoOutput.class);
	}
