 ...
 * 
 */
public class ExportItemPedidoImpl {
	...
	public ExportItemPedidoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ExportItemPedidoInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				BasicoConstants.DOMAIN, //
				BasicoConstants.SERVICE, //
				BasicoConstants.Commands.EXPORT_ITEM_PEDIDO, //
	...
	*/
	public ExportItemPedidoOutput exportItemPedido(ExportItemPedidoInput input, long timeout) {
		...
		ExportItemPedidoOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ExportItemPedidoOutput.class);
		if (output == null) {
	...
	*/
	public CompletableFuture<ExportItemPedidoOutput> exportItemPedidoRequest(ExportItemPedidoInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ExportItemPedidoOutput.class);
	}
